...
try {
    ...
    if(isset($postData['nom'], $postData['desc'], $postData['imglst'], $postData['id'])){
        include("infosbase.php");

        // Préparer et exécuter la requête SQL pour mettre à jour l'habitat
        $stmt = $conn->prepare("UPDATE `habitat` SET `Nom`=?, `images_liste`=?, `description`=? WHERE `ID`=?");
        $stmt->execute([$postData['nom'], $postData['imglst'], $postData['desc'], $postData['id']]);
